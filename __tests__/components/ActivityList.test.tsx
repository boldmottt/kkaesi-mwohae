...
const baseProps = { profileId: 'test-profile', date: '2026-04-22', windowIndex: 0 }

describe('ActivityList', () => {
  it('활동 이름을 렌더링한다', () => {
    render(<ActivityList activities={activities} loading={false} {...baseProps} />)
    expect(screen.getByText('터미타임')).toBeInTheDocument()
    expect(screen.getByText('흑백 카드')).toBeInTheDocument()
  })

  it('로딩 중에 스켈레톤을 표시한다', () => {
    render(<ActivityList activities={[]} loading={true} {...baseProps} />)
